...
### Compare CSV uid and generated account_id

If you want to verify whether the user identifier in the CSV is already consistent with the `account_id` stored in the `userproof` table, run:

```Plaintext
./main tool compare_account_id
```

To check a specific CSV file instead of `UserDataFile` in `config.json`, run:

```Plaintext
./main tool compare_account_id --file ./example_data/example_users.csv
```

This command reads the second column of each CSV row, applies the same conversion as the `userproof` generation flow, and prints whether the original uid matches the generated `account_id`.
