...
func CexVerify() error {
	...
	if err != nil {
		return fmt.Errorf("unmarshal proof csv failed: %w", err)
	}
	...
	emptyAccountTreeRoot, err := hex.DecodeString("15a663ddcc8e79ff217a026669f2846142b046c21e2410a4a15d03c755856ce5") // 64
	// emptyAccountTreeRoot, err := hex.DecodeString("280ee8f1e2abd276ce5b697cfad42f8d0d610831c95bce67c3e1913dc813146a") // 618
	// emptyAccountTreeRoot, err := hex.DecodeString("2c939165c8e8faaf4da574c47cc2a4a06fcabcd0980d09b3e1cd6c15913460c3") // 621

	if err != nil {
		return fmt.Errorf("invalid empty account tree root: %w", err)
	}
	...
	for i := 0; i < len(cexConfig.CexAssetsInfo); i++ {
		...
		if cexConfig.CexAssetsInfo[i].TotalEquity < cexConfig.CexAssetsInfo[i].TotalDebt {
			...
			return fmt.Errorf("invalid cex asset info: %s equity less than debt", cexConfig.CexAssetsInfo[i].Symbol)
		}
	}
	...
	for i := 0; i < len(proofs); i++ {
		if batchNumber != proofs[i].BatchNumber {
			return fmt.Errorf("batch number is not monotonically increasing by 1")
		}
		...
		if err != nil {
			return fmt.Errorf("decode proof failed for batch %d: %w", batchNumber, err)
		}
		...
		for j := 0; j < len(proofs[i].CexAssetCommitment); j++ {
			...
			if err != nil {
				return fmt.Errorf("decode cex asset commitment failed for batch %d: %w", batchNumber, err)
			}
		}
		fmt.Println("=======================")
		...
		for j := 0; j < len(proofs[i].AccountTreeRoots); j++ {
			...
			if err != nil {
				return fmt.Errorf("decode account tree root failed for batch %d: %w", batchNumber, err)
			}
		}
		...
		if err != nil {
			return fmt.Errorf("decode batch commitment failed for batch %d: %w", batchNumber, err)
		}
		if string(expectHash) != string(actualHash) {
			return fmt.Errorf("public input verify failed for batch %d: expected %x got %x", batchNumber, expectHash, actualHash)
		}
		...
		if string(accountTreeRoots[0]) != string(prevAccountTreeRoots[1]) ||
			...
			return fmt.Errorf("mismatch account tree root or cex asset list commitment for batch %d", batchNumber)
		}
		...
		if err != nil {
			return fmt.Errorf("create verify witness failed for batch %d: %w", batchNumber, err)
		}
		err = groth16.Verify(proof, vk, vWitness)
		if err != nil {
			return fmt.Errorf("proof verify failed for batch %d: %w", batchNumber, err)
		} else {
...
func UserVerify() error {
	userConfig := &config.UserConfig{}
	content, err := ioutil.ReadFile("./config/user_config.json")
	if err != nil {
		return fmt.Errorf("read user config failed: %w", err)
	}
	...
	for i := 0; i < len(userConfig.MerkleProofEncode); i++ {
		...
		if err != nil || len(p) != 32 {
			return fmt.Errorf("invalid proof at index %d", i)
		}
	...
	if err != nil || len(accountIdHash) != 32 {
		return fmt.Errorf("account id hash is invalid")
	}
