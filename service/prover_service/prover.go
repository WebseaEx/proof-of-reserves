...
func LoadVerifyingKey(filepath string) (verifyingKey groth16.VerifyingKey, err error) {
	...
	filepath = filepath + ".vk.save"
	f, err := os.Open(filepath)
	if err != nil {
		return verifyingKey, fmt.Errorf("open verifying key %s failed: %w", filepath, err)
	}
	defer f.Close()
	_, err = verifyingKey.ReadFrom(f)
	if err != nil {
		return verifyingKey, fmt.Errorf("read verifying key %s failed: %w", filepath, err)
	}
