...
func ReadUserDataFromCsvFile(name string) ([]AccountInfo, []CexAssetInfo, error) {
	...
	for i := 0; i < assetCounts; i++ {
		cexAssetsInfo[i].Symbol = symbols[i*4+4]
		...
		cexAssetsInfo[i].BasePrice, err = ConvertFloatStrToUint64(data[0][i*4+5], multiplier)
		if err != nil {
			fmt.Println("asset data wrong:", data[0][i*4+5], err.Error())
			continue
		}
	...
	for i := 0; i < len(data); i++ {
		...
		for j := 0; j < assetCounts; j++ {
			...
			equity, err := ConvertFloatStrToUint64(data[i][j*4+2], multiplier)
			if err != nil {
			...
			debt, err := ConvertFloatStrToUint64(data[i][j*4+3], multiplier)
			if err != nil {
		...
		if account.TotalEquity.Cmp(account.TotalDebt) >= 0 {
			...
			LogMaxDebtGapAsset(data[i][1], assets, cexAssetsInfo)
		}
		if i%100000 == 0 {
			runtime.GC()
		}
...
func LogMaxDebtGapAsset(accountID string, assets []AccountAsset, cexAssetsInfo []CexAssetInfo) {
	maxGap := new(big.Int)
	var maxAsset AccountAsset
	var maxSymbol string
	var maxBasePrice uint64
	maxEquityValue := new(big.Int)
	maxDebtValue := new(big.Int)
	found := false

	for i := 0; i < len(assets); i++ {
		assetIndex := int(assets[i].Index)
		if assetIndex >= len(cexAssetsInfo) {
			continue
		}
		basePrice := cexAssetsInfo[assetIndex].BasePrice
		equityValue := new(big.Int).Mul(new(big.Int).SetUint64(assets[i].Equity), new(big.Int).SetUint64(basePrice))
		debtValue := new(big.Int).Mul(new(big.Int).SetUint64(assets[i].Debt), new(big.Int).SetUint64(basePrice))
		gap := new(big.Int).Sub(debtValue, equityValue)
		if gap.Sign() <= 0 {
			continue
		}

		if !found || gap.Cmp(maxGap) > 0 {
			found = true
			maxGap = gap
			maxAsset = assets[i]
			maxSymbol = cexAssetsInfo[assetIndex].Symbol
			maxBasePrice = basePrice
			maxEquityValue = equityValue
			maxDebtValue = debtValue
		}
	}

	if found {
		fmt.Println("account", accountID, "largest debt-over-equity asset:",
			"index", maxAsset.Index, "symbol", maxSymbol,
			"debt", maxAsset.Debt, "equity", maxAsset.Equity,
			"basePrice", maxBasePrice, "debtValue", maxDebtValue,
			"equityValue", maxEquityValue, "gap", maxGap)
		return
	}

	fmt.Println("account", accountID, "no single asset has debt-over-equity gap; check CSV column alignment and basePrice parsing")
}
...
func ComputeCexAssetsCommitment(cexAssetsInfo []CexAssetInfo) []byte {
	hasher := poseidon.NewPoseidon()
	emptyNum := 0
	if len(cexAssetsInfo) < AssetCounts {
		emptyNum = AssetCounts - len(cexAssetsInfo)
	}
	emptyCexAssets := make([]CexAssetInfo, emptyNum)
