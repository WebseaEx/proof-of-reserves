...
func ReadUserDataFromCsvFileV1(name string) ([]AccountInfo, []CexAssetInfo, error) {
	...
	for i := 0; i < len(data); i++ {
		...
		for j := 0; j < assetCounts; j++ {
			...
			if err != nil {
				fmt.Println("account", data[i][1], "the symbol is ", cexAssetsInfo[j].Symbol, " ", data[i][j*4+2], " equity data wrong:", err.Error())
				invalidCounts += 1
				continue
			}
			...
			if err != nil {
				fmt.Println("account", data[i][1], "the debt symbol is ", cexAssetsInfo[j].Symbol, " ", data[i][j*4+3], " debt data wrong:", err.Error())
				invalidCounts += 1
				continue
			}
			if equity != 0 || debt != 0 {
